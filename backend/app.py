...
def hello_world():
    ...
    for tr_file in translations_to_process:
        ...
        for img_file in images_to_process:
            ...
            for i, (box, text_str) in enumerate(zip(boxes, text_strs)):
    ...
    tmpzip = io.BytesIO()
    with ZipFile(tmpzip, mode='x', compression=ZIP_DEFLATED) as zipfile:
        for i, img_path in enumerate(images_to_process):
            zipfile.write(img_path, secure_filename(images[i].filename))
